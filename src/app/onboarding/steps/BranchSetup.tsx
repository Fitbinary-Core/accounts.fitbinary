...
const BranchSetup = ({ branch_details }: BranchDetailsProps) => {
    ...
    const onSubmit: SubmitHandler<BranchInput> = async (data) => {
        try {
            setLoading(true);
            const response = await create_update_main_branch({ ...data, is_main: true });
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="pt-6 border-t border-gray-100">
                <div className="flex items-center justify-center gap-2 text-gray-400 bg-gray-50 py-2 rounded-lg">
                    <Navigation className="w-3.5 h-3.5" />
                    <span className="text-[10px] font-bold uppercase tracking-widest">
                        Standardized Branch Deployment Protocol
                    </span>
                </div>
            </div>
        </div>
