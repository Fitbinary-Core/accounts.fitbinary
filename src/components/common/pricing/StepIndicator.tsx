      ...
      {steps.map((item, index) => (
        ...
        >
          <div className="flex flex-col items-center top-3 relative z-10 bg-gray-50/30 md:px-4">
            <div
              className={`w-10 h-10 flex items-center justify-center font-semibold text-sm rounded-md border-2 transition-all ${
                activeStep === item.id
            ...
            <span
              className={`text-xs font-semibold uppercase tracking-wider mt-3 ${
                activeStep === item.id ? "text-gray-900" : "text-gray-400"
              }`}
            >
              {item.label}
            </span>
          ...
          {index < steps.length - 1 && (
            <div
              className={`flex-1 h-0.5 -ml-8 -mr-8 border-t-2 border-dashed ${
                index < activeIndex ? "border-red-600" : "border-gray-200"
              }`}
            />
          )}
        </div>
