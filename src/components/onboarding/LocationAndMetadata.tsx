...
const LocationAndMetadata = ({
  ...
  const countryOptions: SelectOption[] = useMemo(() => {
    return (onboarding_fields_data?.address_details?.countries || []).map(
      (c: any) => ({
        value: c.code,
        label: c.name,
        flag: c.flag,
  ...
  const timezoneOptions: SelectOption[] = useMemo(() => {
    return (onboarding_fields_data?.meta_data?.timezones || []).map(
      (tz: any) => ({
        value: tz.value,
        label: tz.label,
  ...
  const currencyOptions: SelectOption[] = useMemo(() => {
    return (onboarding_fields_data?.meta_data?.currencies || []).map(
      (c: any) => ({
        value: c.value,
        label: c.label,
        symbol: c.symbol,
